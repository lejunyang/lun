...
export class VueElement extends BaseClass {
	...
	private _applyStyles(styles: (string | CSSStyleSheet)[] | undefined) {
		if (styles && !this._def.noShadow) {
			if (CSSStyleSheet instanceof Function && 'adoptedStyleSheets' in document) {
				this.shadowRoot!.adoptedStyleSheets = styles.map((css) => {
					const styleSheet = typeof css === 'string' ? new CSSStyleSheet() : css;
					typeof css === 'string' && styleSheet.replaceSync(css);
					return styleSheet;
				});
			} else {
				styles.forEach((css) => {
